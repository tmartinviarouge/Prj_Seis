...
int Temperature::MesureTempEnDixieme() {
    int Temperature = wiringPiI2CReadReg16(fd, AMBIENT_TEMPERATURE_REGISTER); // Mesure de la température Ambiante
    int degre = (Temperature & 0xff00) >> 12; //12 bits de décalage pour que les poids faibles et poids forts soit au bon endroit
    int dixieme = Temperature & 0x00ff; // Car WiringPi inverse LSB et MSB
     //cout << "Température ambiante : " << dixieme << "," << degre << "C°" << endl;
    return degre,dixieme;
}
